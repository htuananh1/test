...
def short_number(n):
    if n >= 1_000_000:
        return f"{n//1_000_000}tr"
    if n >= 1_000:
        return f"{n//1_000}k"
    return str(n)

def format_number(n):
    return short_number(n)
...
⏰ Reset xu: Chủ nhật {next_reset.strftime('%d/%m %H:%M')}
...
async def show_menu(update_or_query, user_id):
    ...
    next_reset = get_next_sunday()
    keyboard = [
        [InlineKeyboardButton("🎣 Câu Cá", callback_data='game_fishing'), InlineKeyboardButton("🤖 Auto", callback_data='auto_fishing')],
        [InlineKeyboardButton("🎣 Cần Câu", callback_data='shop_rods'), InlineKeyboardButton("🗺️ Kho Báu", callback_data='game_treasure')],
        [InlineKeyboardButton("🎒 Kho Đồ", callback_data='view_inventory'), InlineKeyboardButton("🎲 Chẵn Lẻ", callback_data='game_chanle')],
        [InlineKeyboardButton("📊 Thống Kê", callback_data='view_stats'), InlineKeyboardButton("🏆 BXH", callback_data='leaderboard')],
        [InlineKeyboardButton("🏆 Rank", callback_data='view_rank'), InlineKeyboardButton("🎁 Quà", callback_data='daily_reward')]
    ]
    menu_text = f"""
🎮 **MENU CHÍNH** 🎮
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    data = query.data
    if data == 'shop_rods':
        await shop_rods_callback(query)
    elif data.startswith('select_rod_'):
        rod_id = data.split('_')[-1]
        await select_rod_callback(query, rod_id)
    elif data == 'view_inventory':
        await view_inventory_callback(query)
    elif data == 'sell_fish':
        await sell_fish_callback(query)
    elif data == 'game_chanle':
        await game_chanle_callback(query)
    elif data == 'chanle_even':
        await chanle_result_callback(query, 'even')
    elif data == 'chanle_odd':
        await chanle_result_callback(query, 'odd')
    elif data == 'game_treasure':
        await game_treasure_callback(query)
    elif data == 'view_stats':
        await stats_callback(query)
    elif data == 'leaderboard':
        await leaderboard_callback(query)
    elif data == 'view_rank':
        await rank_callback(query)
    elif data == 'game_fishing':
        await fish_callback(query)
    elif data == 'auto_fishing':
        await auto_fishing_callback(query, context)
    elif data == 'stop_auto':
        await stop_auto_callback(query)
    elif data == 'back_menu':
        await show_menu(query, user_id)
    else:
        await show_menu(query, user_id)

async def shop_rods_callback(query):
    user_id = query.from_user.id
    user = data_manager.get_user(user_id)
...
async def select_rod_callback(query, rod_id):
    user_id = query.from_user.id
    user = data_manager.get_user(user_id)
    owned_rods = user.get('owned_rods', ["1"])
    coins = user['coins']
...
async def view_inventory_callback(query):
    user_id = query.from_user.id
    user = data_manager.get_user(user_id)
...
async def sell_fish_callback(query):
    user_id = query.from_user.id
    user = data_manager.get_user(user_id)
    current_rank, _, _, _ = get_user_rank(user.get('total_exp', 0))
    total_value = 0
        ...
        for fish_name, count in fish_inv.items():
            for fish_type, fish_data in FISH_TYPES.items():
                if fish_type == fish_name:
                    total_value += int(fish_data['value'] * count * 0.7 * current_rank['coin_bonus'])
                    total_count += count
                    break
        user['inventory']['fish'] = {}
...
async def game_chanle_callback(query):
    user_id = query.from_user.id
    user = data_manager.get_user(user_id)
    coins = user['coins']
    text = f"🎲 **Chẵn lẻ** (Tài/Xỉu)\n\n💰 Xu: {format_number(coins)}\nChọn cược:"
    keyboard = [
        [InlineKeyboardButton("⚪ Chẵn", callback_data='chanle_even'), InlineKeyboardButton("🔴 Lẻ", callback_data='chanle_odd')],
        [InlineKeyboardButton("↩️ Menu", callback_data='back_menu')]
    ]
    await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='Markdown')

async def chanle_result_callback(query, bet_type):
    user_id = query.from_user.id
    user = data_manager.get_user(user_id)
    coins = user['coins']
    bet_amount = 50
...
async def game_treasure_callback(query):
    user_id = query.from_user.id
    user = data_manager.get_user(user_id)
    coins = user['coins']
    if coins < 100:
...
async def stats_callback(query):
    ...
    text = f"""📊 **THỐNG KÊ** 📊
👤 {user['username']}
🎣 Số lần câu: {format_number(user.get('fishing_count',0))}
🏆 Thắng: {format_number(user.get('win_count',0))}
❌ Thua: {format_number(user.get('lose_count',0))}
🪙 Kho báu: {format_number(user.get('treasures_found',0))}
💰 Xu: {format_number(user['coins'])}
⭐ EXP: {format_number(user.get('total_exp',0))}
"""
...
async def leaderboard_callback(query):
    all_users = []
    try:
...
async def fish_callback(query):
    ...
    result, error = await process_fishing(user_id)
    if error:
        await query.edit_message_text(error)
        return
    current_rank, _, _, _ = get_user_rank(user.get('total_exp', 0))
    if result["success"]:
        result_text = f"""🎉 **BẮT ĐƯỢC!**
...
async def auto_fishing_callback(query, context):
    user_id = query.from_user.id
    user = data_manager.get_user(user_id)
    ...
    await query.edit_message_text("🤖 **BẮT ĐẦU AUTO...**\nSẽ dừng khi bạn hết xu hoặc bạn nhấn Dừng.")
    asyncio.create_task(auto_fishing_task(query, context, user_id))

async def stop_auto_callback(query):
    user_id = query.from_user.id
    if user_id in data_manager.auto_fishing_tasks:
        data_manager.auto_fishing_tasks[user_id] = False
    await query.edit_message_text("🛑 **ĐÃ DỪNG AUTO**")
    await asyncio.sleep(1)
    await show_menu(query, user_id)

async def auto_fishing_task(query, context, user_id):
