...
async def def generate_image(prompt: str) -> bytes | None:
    """
    Generate image bytes with Gemini 2.0 Flash Preview Image Generation.
    Returns bytes or None on failure. Safe for Telegram send_photo.
    """
    try:
        # Lazy import for safety in environments without the lib
        try:
            from google import genai
        except Exception as _e:
            logger.error("Gemini SDK not available: %s", _e)
            return None
        ...
        for attempt in range(3):
            try:
                response = client.models.generate_image(
                    ...
                    prompt=prompt.strip()[:800],  # avoid overly long prompts
                    number_of_images=1,
                ...
                if not hasattr(response, "images") or not response.images:
                    last_err = RuntimeError("No images returned")
                    continue
                for image in response.images:
                    # Prefer the public attribute if available
                    data = getattr(image, "image_bytes", None) or getattr(image, "_image_bytes", None)
                    if isinstance(data, (bytes, bytearray)):
                        return bytes(data)
                last_err = RuntimeError("Image object missing bytes")
            except Exception as e:
                last_err = e
                logger.warning("generate_image attempt %d failed: %s", attempt+1, e)
                asyncio.sleep(0.2)
        logger.error("generate_image failed after retries: %s", last_err)
        return None
    except Exception as e:
        logger.exception("generate_image fatal error: %s", e)
        return None
...
async def img_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Telegram-safe /img command with prompt args and robust sending."""
    user_id = update.effective_user.id
    prompt_text = " ".join(context.args).strip() if context.args else ""
    if not prompt_text and update.message and update.message.reply_to_message:
        prompt_text = (update.message.text or "").replace("/img", "").strip()

    if not prompt_text:
        user_states[user_id] = "waiting_image_prompt"
        await update.message.reply_text(
            "🖼 **TẠO ẢNH VỚI AI**\n\nMô tả hình ảnh bạn muốn tạo (ví dụ: *"mèo máy Doraemon ngồi ăn bánh rán, phong cách anime"*).",
            parse_mode=ParseMode.MARKDOWN
        )
        return

    try:
        # Show typing/upload action to keep bot responsive
        await context.bot.send_chat_action(chat_id=update.effective_chat.id, action=ChatAction.UPLOAD_PHOTO)
    except Exception:
        pass

    img_bytes = await asyncio.get_event_loop().run_in_executor(None, generate_image, prompt_text)
    if not img_bytes:
        await update.message.reply_text("⚠️ Xin lỗi, tạo ảnh thất bại. Hãy thử mô tả khác hoặc ngắn gọn hơn.")
        return

    # Try send as photo first; fallback to document if needed
    file_name = "image.jpg"
    bio = io.BytesIO(img_bytes)
    bio.name = file_name
    bio.seek(0)
    try:
        await update.message.reply_photo(photo=bio, caption=f"✅ Ảnh đã tạo từ mô tả: _{prompt_text}_", parse_mode=ParseMode.MARKDOWN)
        return
    except Exception as e:
        logger.warning("send_photo failed, falling back to document: %s", e)

    bio.seek(0)
    try:
        await update.message.reply_document(document=bio, filename=file_name, caption=f"✅ Ảnh đã tạo từ mô tả: _{prompt_text}_", parse_mode=ParseMode.MARKDOWN)
    except Exception as e:
        logger.error("send_document also failed: %s", e)
        await update.message.reply_text("❌ Không thể gửi ảnh do giới hạn Telegram hoặc lỗi tệp.")
...
async def __orig_math_game(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if hasattr(update, 'callback_query'):
        user_id = update.callback_query.from_user.id
        message = update.callback_query.message
    else:
...
async def math_game(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Hardened math_game for Telegram: guards state, avoids KeyErrors, edits messages safely."""
    try:
        if hasattr(update, "callback_query") and update.callback_query:
            ...
            message = update.message
            data = ""
        # ensure game dict exists
        user_games.setdefault(user_id, {})
        game_manager = GameManager()

        # delegate to original implementation if it exists (wrapped below)
        return await __orig_math_game(update, context)
    except Exception as e:
        logger.exception("math_game error: %s", e)
        try:
            if hasattr(update, "callback_query") and update.callback_query:
                await update.callback_query.answer("Đã xảy ra lỗi, vui lòng thử lại.", show_alert=True)
            else:
                await message.reply_text("Đã xảy ra lỗi, vui lòng thử lại.")
        except Exception:
            pass

async def __orig_riddle_game(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if hasattr(update, 'callback_query'):
        user_id = update.callback_query.from_user.id
        message = update.callback_query.message
    else:
...
async def riddle_game(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Hardened riddle_game for Telegram: guards state, avoids KeyErrors, edits messages safely."""
    try:
        if hasattr(update, "callback_query") and update.callback_query:
            user_id = update.callback_query.from_user.id
            message = update.callback_query.message
        ...
        # delegate to original implementation if it exists (wrapped below)
        return await __orig_riddle_game(update, context)
    except Exception as e:
        logger.exception("riddle_game error: %s", e)
        try:
            if hasattr(update, "callback_query") and update.callback_query:
                await update.callback_query.answer("Đã xảy ra lỗi, vui lòng thử lại.", show_alert=True)
            else:
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if await handle_waiting_image_prompt(update, context):
        return
...
async def handle_waiting_image_prompt(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    """If user is in waiting_image_prompt state, consume message as prompt for /img."""
    user_id = update.effective_user.id
    if user_states.get(user_id) == "waiting_image_prompt":
        user_states[user_id] = None
        # Reuse img_command logic by injecting args
